<template>
  <section class="wiki-entity-list">
    <div v-if="visibleFactions.length" class="entity-grid">
      <article v-for="faction in visibleFactions" :key="faction.name" class="entity-card">
        <div class="entity-card__icon">
          <i class="mdi mdi-flag-variant-outline"></i>
        </div>
        <div class="entity-card__content">
          <h2>{{ faction.name || '未命名势力' }}</h2>
          <p>{{ formatAliases(faction.aliases) }}</p>
          <v-btn class="story-button" variant="flat" size="small" @click="openStory(faction)">
    ...
    <div v-else class="empty-state">暂无势力数据</div>

    <div ref="sentinelRef" class="load-sentinel">
      <span v-if="visibleFactions.length < normalizedFactions.length">继续下滑加载更多势力</span>
      <span v-else-if="visibleFactions.length">已加载全部势力</span>
    </div>

    <v-dialog v-model="storyDialogVisible" max-width="720">
      <div class="story-dialog">
        <div class="story-dialog__header">
          <h2>{{ selectedEntity?.name || '势力剧情' }}</h2>
...
const normalizedFactions = computed(() => normalizeList(props.factions));
const visibleFactions = computed(() => normalizedFactions.value.slice(0, visibleCount.value));
...
function loadMore() {
  if (visibleCount.value >= normalizedFactions.value.length) {
    return;
  }
